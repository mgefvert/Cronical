...
public partial class Service : ServiceBase
{
    ...
    private readonly List<IIntegration> _integrations = new();
    ...
    {
        Log.Information("Process startup");
        Log.Information($"Using definition file {_configFilename}");

        // One FileConfigReader is always required
        _fileConfigReader = new FileConfigReader(_configFilename);
        _fileConfigReader.Initialize(_globalSettings);
    ...
    private IEnumerable<IIntegration> LoadIntegration(string integrationName)
    {
        ...
        foreach (var type in assembly.GetTypes().Where(t => t.IsInstanceOfType(typeof(IIntegration))))
        {
            ...
            {
                Log.Error($"Exception {e.GetType().Name} while loading integration '{integrationName}': {e.Message}");
            }

            if (integration != null && integration.Initialize(_globalSettings))
