...
internal class InjectCtrlC
{
    ...
    internal static void Break(int processId)
    {
        ...
        if (result == null)
            Log.Warning("CtrlC: Unable to start kill process: " + cmd);
    }
    ...
    internal static void Handle(string[] args)
    {
        ...
        Log.Information("CtrlC: Sending BREAK to process " + pid);

        FreeConsole();
        if (!AttachConsole((uint)pid))
        {
            Log.Debug("Unable to attach to console for process " + pid);
            return;
        }
